...
# Initialize client inside functions to avoid import issues during DAG parsing
...
def write_parquet_to_minio(df: pd.DataFrame, bucket: str, path: str):
    ...
    try:
        ...
        print(f"Warning: could not ensure {bucket} bucket exists: {e}")
...
def validate_gold_data_quality(**context):
    """Validate gold layer dimension and fact tables for DA team"""
    ti = context["ti"]
    gold_ts = ti.xcom_pull(key="gold_ts", task_ids="detect_latest_silver_folders")
    ...
    try:
        ...
        # Expected gold files (dimension and fact tables)
        expected_files = [
            'dim_customers.parquet',
            'dim_products.parquet', 
            'dim_sellers.parquet',
        ...
        print(f"🏆 VALIDATING GOLD LAYER DATA")
        print(f"📁 Folder: gold/{gold_ts}/")
        ...
        for filename in expected_files:
            try:
                object_path = f"gold/{gold_ts}/{filename}"
                df = read_parquet_from_minio(GOLD_BUCKET, object_path)
                
                # Get file size
                obj_stat = client.stat_object(GOLD_BUCKET, object_path)
                size_mb = obj_stat.size / (1024 * 1024)
                total_size_mb += size_mb
                
                print(f"📊 {filename}: {len(df):,} rows, {len(df.columns)} cols, {size_mb:.2f} MB")
                
                # Basic validation
                if len(df) > 0 and len(df.columns) > 0:
                    # Check primary key for dimension tables
                    if filename.startswith('dim_'):
                        pk_column = df.columns[0]
                        duplicates = df[pk_column].duplicated().sum()
                        nulls = df[pk_column].isnull().sum()
                        if duplicates == 0 and nulls == 0:
                            print(f"  ✅ Dimension table structure PASSED")
                        else:
                            print(f"  ⚠️ {duplicates} duplicates, {nulls} nulls in PK")
                    
                    # Check fact table measures
                    elif filename.startswith('fact_'):
                        numeric_cols = df.select_dtypes(include=['number']).columns.tolist()
                        print(f"  📈 Measures: {len(numeric_cols)} numeric columns")
                        if 'revenue' in df.columns:
                            revenue_stats = df['revenue'].describe()
                            print(f"  💰 Revenue: ${revenue_stats['mean']:.2f} avg, ${revenue_stats['max']:.2f} max")
                        print(f"  ✅ Fact table structure PASSED")
        ...
        print(f"  📊 Files: {validated_files}/{len(expected_files)} validated")
        print(f"  💾 Total size: {total_size_mb:.2f} MB")
        print(f"  🎯 Location: MinIO gold bucket, folder: gold/{gold_ts}/")
        print(f"  ✅ Data ready for DA team Power BI dashboards!")
        
        if validated_files < len(expected_files):
            print(f"  ⚠️ Some files missing, but core tables available")
...
with DAG(
    ...
    # graph: detect -> build dims/date/reviews in parallel -> fact -> validate -> finalize
    t_detect >> [t_dim_customers, t_dim_products, t_dim_sellers, t_dim_date, t_dim_reviews] >> t_fact_orders >> t_validate >> t_finalize
